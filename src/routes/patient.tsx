...
import { Home, Pill, FlaskConical, Activity, Calendar, Settings, LogOut, MessageSquare } from "lucide-react";
...
function PatientLayout() {
  ...
  if (role && role !== "patient") { throw redirect({ to: "/doctor" }); }

  const nav = [
    { to: "/patient", label: t("dashboard"), icon: Home },
    { to: "/patient/medications", label: t("medications"), icon: Pill },
    { to: "/patient/labs", label: t("labTests"), icon: FlaskConical },
    { to: "/patient/glucose", label: t("glucose"), icon: Activity },
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium whitespace-nowrap transition ${
                  active ? "gradient-primary text-primary-foreground shadow-glow" : "hover:bg-muted"
                }`}
              >
